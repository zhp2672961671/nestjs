...
const log4jsConfig = {
  appenders: {
    ...
    nest: {
      type: 'dateFile',	// 写入文件格式，并按照日期分类
      filename: `${baseLogPath}/nest/nest.log`,	// 日志文件名，会命名为：access.2021-04-01.log
      alwaysIncludePattern: true,	// 为true, 则每个文件都会按pattern命名，否则最新的文件不会按照pattern命名
      pattern: 'yyyy-MM-dd',	// 日期格式
      // maxLogSize: 10485760,  // 日志大小
      daysToKeep: 30,	// 文件保存日期30天
  ...
  categories: {
    ...
    mysql: { appenders: ['access', 'errors','console'], level: 'info' },
    http: { appenders: ['access'], level: 'DEBUG' },
    nest:{appenders: ['nest','console','errors'], level: 'DEBUG'}
  },
};
